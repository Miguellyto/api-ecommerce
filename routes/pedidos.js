...
router.get('/', (req, res, next) => {// req=Requisição, res=Respota
...
router.post('/', (req, res, next) => {
    /// Caso não encontre o produto para fazer o pedido
    mysql.getConnection((error, conn) =>{
       if (error) { return res.status(500).send({ error: error}) }
       ...
       [req.body.id_produto], (error, resultado, field) => {
        ...
        /// Caso encontre o produto para fazer o pedido
        conn.query(
...
if (resultado.length == 0){
    return res.status(404).send({
        mensagem: 'Não foi encotrado pedido'
    })
}
const response = {
       pedido: {
